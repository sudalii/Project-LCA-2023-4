(function($) {
    ...
    function updateContent(categoryName) {
        ...
        category.flows.forEach(flow => {
            if (flow.impactResult !== 0) { // impactResult가 0이 아닌 경우에만 표시
                contentHtml += '<tr>';
                contentHtml += '<td>' + flow.name + '</td>';
                contentHtml += '<td>' + flow.cf + ' kg' + '</td>';
                contentHtml += '<td>' + formatNumber(flow.impactResult) + ' ' + category.result.unit + '</td>';
                contentHtml += '</tr>';
            }
        });
        ...
        const impactValueHtml = '➜ 총 환경영향값: ' + formatNumber(category.result.value) + ' ' + category.result.unit;
