...
public class Database {
    ...
    public static IDatabase activate(DatabaseConfig config) {
        try {
            database = config.connect(DataDir.get().getDatabasesDir());
            ...
            Logger log = LoggerFactory.getLogger(Database.class);
            log.trace("failed to activate database: " + config, e);
            return null;
        }
    }
    ...
    /*
    Workspace == DataDir.get()
    Workspace.dbDir() == DataDir.get().getDatabasesDir()
    DatabaseDir.getRootFolder(local.name()) == new File(DataDir.get().getDatabasesDir(), local.name())
     */
    private static DatabaseConfigList readConfigs() {
        var file = new File(DataDir.get().root(), "databases.json");
        if (!file.exists())
            return new DatabaseConfigList();

        var configs = DatabaseConfigList.read(file);
/*        var configList = configs.getDerbyConfigs();

        if (config instanceof MySqlConfig) {
            configList = (List<MySqlConfig>) configs.getMySqlConfigs();
        }
        ...
        for (var local : configs.getDerbyConfigs()) {
            var dbDir = new File(DataDir.get().getDatabasesDir(), local.name());
            found.add(local.name());
            if (!dbDir.exists()) {
                // we do not delete the config currently
                LoggerFactory.getLogger(Database.class).warn(
                        "Registered database '{}' does not exist in the workspace.",
                        local.name());
            }
        }

        // add configs for non-registered databases
        var dbDirs = DataDir.get().getDatabasesDir().listFiles();
        var updated = false;
        if (dbDirs != null) {
...
/*    public static String validateNewName(String name) {
        ...
        try {
            var vDir = new File(DataDir.get().getDatabasesDir(), name);
